<?php
// FILE: cricket.php (Fully Functional with Appeals, Comments, and Team Details in Cards)
session_start();
...
$sport_id_for_cricket = 1; // Cricket's sport_id is 1 (as per your database dump)
    ...
    if (isset($_POST['submit_appeal'])) {
        ...
        if ($event_id > 0 && !empty($reason)) {
            ...
            if ($check_stmt->get_result()->num_rows > 0) {
                $_SESSION['form_status'] = ['page' => 'cricket', 'type' => 'error', 'message' => 'You have already submitted an appeal for this match.'];
            } else {
                ...
                $_SESSION['form_status'] = $stmt->execute() ? ['page' => 'cricket', 'type' => 'success', 'message' => 'Your appeal has been submitted successfully.'] : ['page' => 'cricket', 'type' => 'error', 'message' => 'Error: Could not submit your appeal.'];
                $stmt->close();
            }
            $check_stmt->close();
        } else {
            $_SESSION['form_status'] = ['page' => 'cricket', 'type' => 'error', 'message' => 'Error: Please provide a reason for your appeal.'];
        }
...
$stmt_all_upcoming->bind_param("i", $sport_id_for_cricket);
...
$stmt_results_dashboard->bind_param("i", $sport_id_for_cricket);
...
$stmt_all_results->bind_param("i", $sport_id_for_cricket);
...
$stmt_points->bind_param("iii", $sport_id_for_cricket, $sport_id_for_cricket, $sport_id_for_cricket);
$stmt_points->execute();
...
$stmt_teams_count->bind_param("i", $sport_id_for_cricket);
$stmt_teams_count->execute();
$total_teams = $stmt_teams_count->get_result()->fetch_assoc()['total'];
$stmt_teams_count->close();

// NEW: Fetch all Cricket teams for the "Teams" tab with player count
$stmt_cricket_teams = $conn->prepare("
    SELECT 
        t.team_id, 
        t.team_name,
...
");
$stmt_cricket_teams->bind_param("i", $sport_id_for_cricket);
$stmt_cricket_teams->execute();
$cricket_teams = $stmt_cricket_teams->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt_cricket_teams->close();
...
<head>
    ...
    <title>Cricket Dashboard – SportsAcademix</title>
...
<body>
    <div class="page-container">
        ...
        <?php if (isset($_SESSION['form_status']) && $_SESSION['form_status']['page'] === 'cricket') { $status = $_SESSION['form_status']; echo '<div class="status-message ' . ($status['type'] === 'success' ? 'status-success' : 'status-error') . '">' . htmlspecialchars($status['message']) . '</div>'; unset($_SESSION['form_status']); } ?>
        ...
        <div class="tab-content-container">
            ...
            <div id="teams" class="tab-pane">
                <div class="card-grid">
                    <?php if (empty($cricket_teams)): ?>
                        <p class="empty-state">No teams registered for Cricket yet.</p>
                    <?php else: ?>
                        <?php foreach ($cricket_teams as $team): ?>
                            <div class="team-card">
