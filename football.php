<?php
// FILE: football.php (Fully Functional with Appeals, Comments, and Team Details in Cards)
session_start();
...
$sport_id_for_this_page = 2; // Football's sport_id is 2 (as per your database dump)
    ...
    if (isset($_POST['submit_appeal'])) {
        ...
        if ($event_id > 0 && !empty($reason)) {
            ...
            if ($check_stmt->get_result()->num_rows > 0) {
                $_SESSION['form_status'] = ['page' => 'football', 'type' => 'error', 'message' => 'You have already submitted an appeal for this match.'];
            } else {
                ...
                $_SESSION['form_status'] = $stmt->execute() ? ['page' => 'football', 'type' => 'success', 'message' => 'Your appeal has been submitted successfully.'] : ['page' => 'football', 'type' => 'error', 'message' => 'Error: Could not submit your appeal.'];
                $stmt->close();
            }
            $check_stmt->close();
        } else {
            $_SESSION['form_status'] = ['page' => 'football', 'type' => 'error', 'message' => 'Error: Please provide a reason for your appeal.'];
        }
...
// NEW: Fetch all Football teams for the "Teams" tab with player count
...
<head>
    ...
    <title>Football Dashboard – SportsAcademix</title>
...
<body>
    <div class="page-container">
        ...
        <?php if (isset($_SESSION['form_status']) && $_SESSION['form_status']['page'] === 'football') { $status = $_SESSION['form_status']; echo '<div class="status-message ' . ($status['type'] === 'success' ? 'status-success' : 'status-error') . '">' . htmlspecialchars($status['message']) . '</div>'; unset($_SESSION['form_status']); } ?>
            ...
            <div id="teams" class="tab-pane">
                <div class="card-grid">
                    <?php if (empty($sport_teams)): ?>
                        <p class="empty-state">No teams registered for Football yet.</p>
                    <?php else: ?>
