<?php
// FILE: basketball.php (Fully Functional with Appeals, Comments, and Team Details in Cards)
session_start();
...
$sport_id_for_this_page = 5; // Basketball's sport_id is 5 (as per your database dump)
    ...
    if (isset($_POST['submit_appeal'])) {
        ...
        if ($event_id > 0 && !empty($reason)) {
            ...
            if ($check_stmt->get_result()->num_rows > 0) {
                $_SESSION['form_status'] = ['page' => 'basketball', 'type' => 'error', 'message' => 'You have already submitted an appeal for this match.'];
            } else {
                ...
                $_SESSION['form_status'] = $stmt->execute() ? ['page' => 'basketball', 'type' => 'success', 'message' => 'Your appeal has been submitted successfully.'] : ['page' => 'basketball', 'type' => 'error', 'message' => 'Error: Could not submit your appeal.'];
                $stmt->close();
            }
            $check_stmt->close();
        } else {
            $_SESSION['form_status'] = ['page' => 'basketball', 'type' => 'error', 'message' => 'Error: Please provide a reason for your appeal.'];
        }
...
// NEW: Fetch all Basketball teams for the "Teams" tab with player count
...
<head>
    ...
    <title>Basketball Dashboard – SportsAcademix</title>
    ...
    <div class="page-container">
        <header class="page-header animate-on-load">
            <div class="header-title">
                <i class="fas fa-basketball-ball"></i> <!-- Basketball icon -->
                <div>
        ...
        <?php if (isset($_SESSION['form_status']) && $_SESSION['form_status']['page'] === 'basketball') { $status = $_SESSION['form_status']; echo '<div class="status-message ' . ($status['type'] === 'success' ? 'status-success' : 'status-error') . '">' . htmlspecialchars($status['message']) . '</div>'; unset($_SESSION['form_status']); } ?>
            ...
            <div id="teams" class="tab-pane">
                <div class="card-grid">
                    <?php if (empty($sport_teams)): ?>
                        <p class="empty-state">No teams registered for Basketball yet.</p>
                    <?php else: ?>
